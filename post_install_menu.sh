...
set_user_quota() {
    ...
    if [[ "$_fs_type" == "xfs" ]]; then
        # XFS: use xfs_quota to check user quota state
        if command -v xfs_quota &>/dev/null; then
            local _xfs_state
            _xfs_state=$(xfs_quota -x -c 'state -u' "$mount_point" 2>/dev/null || true)
            if echo "$_xfs_state" | grep -qi "user quota.*on\|Enforcement.*ON\|Accounting.*ON"; then
                _quota_enabled=true
            fi
        fi
        # Fallback: check /proc/mounts for uquota/usrquota
        if [[ "$_quota_enabled" == "false" ]]; then
            if grep -E "^[^ ]+ ${mount_point} " /proc/mounts 2>/dev/null | grep -qE 'usrquota|uquota'; then
                _quota_enabled=true
            fi
        fi
    else
        ...
        if [[ "$_fs_type" == "xfs" ]]; then
            if yes_no "Quotas Not Enabled" "User quotas are not enabled on $mount_point.\n\nFilesystem: XFS\n\nXFS requires the 'uquota' mount option.\nThis needs a remount to activate.\n\nNFS server will be temporarily stopped\nand restarted. Client connections will\nbe briefly interrupted.\n\nEnable quotas now?"; then
                ...
                if grep -q " ${mount_point} " /etc/fstab 2>/dev/null; then
                    local _fstab_line
                    _fstab_line=$(grep " ${mount_point} " /etc/fstab | head -1 || true)
