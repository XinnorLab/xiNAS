...
# Source GDS state library (shared between verify_gds and the Python
# check_gds heredoc in client_healthcheck.sh).
# shellcheck source=lib/gds_state.sh
if [[ -f "$SCRIPT_DIR/lib/gds_state.sh" ]]; then
    source "$SCRIPT_DIR/lib/gds_state.sh"
elif [[ -f "/usr/local/bin/lib/gds_state.sh" ]]; then
    source "/usr/local/bin/lib/gds_state.sh"
elif [[ -f "/opt/xinas-client/lib/gds_state.sh" ]]; then
    source "/opt/xinas-client/lib/gds_state.sh"
else
    echo -e "${RED}Error: lib/gds_state.sh not found${NC}"
    exit 1
fi
