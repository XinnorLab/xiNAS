# IP Pool Network Configuration — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add an IP Pool configuration screen to xinas-menu Network Settings that detects high-speed interfaces, allocates IPs from a pool, auto-detects MTU, and applies directly via netplan.

**Architecture:** New `IPPoolScreen` in `xinas_menu/screens/ip_pool.py` with NavigableMenu + ScrollableTextView layout. Pool config stored in `/etc/xinas/network-pool.json` with atomic writes. On apply, generates `/etc/netplan/99-xinas-pool.yaml` and runs `netplan apply`.

**Tech Stack:** Python 3.10+, Textual TUI framework, PyYAML (for netplan output), JSON (for config storage)

---

### Task 1: Create IPPoolScreen with menu and config helpers

**Files:**
- Create: `xinas_menu/screens/ip_pool.py`

**Step 1: Create the full IPPoolScreen file**
...
Run: `python3 -m py_compile xinas_menu/screens/ip_pool.py`
Expected: No output (success)

**Step 3: Commit**

```bash
git add xinas_menu/screens/ip_pool.py
git commit -m "feat(xinas-menu): add IPPoolScreen with pool config, preview, and netplan apply"
```

---

### Task 2: Wire IPPoolScreen into NetworkScreen

**Files:**
- Modify: `xinas_menu/screens/network.py:22-28` (menu items) and `:49-60` (handler)

**Step 1: Add menu item [5] and import**

In `xinas_menu/screens/network.py`, update the `_MENU` list to add a separator and the IP Pool option before the Back item:
...
**Step 2: Add handler for key "5"**

In the `on_navigable_menu_selected` method, add the case for key `"5"`:
...
Run: `python3 -m py_compile xinas_menu/screens/network.py`
Expected: No output (success)

**Step 4: Commit**

```bash
git add xinas_menu/screens/network.py
git commit -m "feat(xinas-menu): wire IP Pool screen into Network Settings menu"
```
...
```bash
python3 -m py_compile xinas_menu/screens/ip_pool.py
python3 -m py_compile xinas_menu/screens/network.py
```
