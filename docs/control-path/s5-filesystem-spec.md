...
**Amended (hardware smoke, 2026-07-10):** that compatibility table was wrong about capabilities, not only about paths. `mkfs.xfs` (`ioctl(BLKBSZSET)` + `BLKFLSBUF`) and `xfs_growfs` (`XFS_IOC_FSGROWFSDATA`) each make a `capable(CAP_SYS_ADMIN)` check that uid-0 ownership of `/dev/xi_*` does not satisfy. The real delta is `ReadWritePaths=/etc/systemd/system` **plus** `CAP_SYS_ADMIN`, granted in both `CapabilityBoundingSet` and `AmbientCapabilities` — bounding is what a root-exec'd `mkfs.xfs` actually inherits (`pP' = bounding ∪ pI`), ambient keeps that true if the unit ever stops running as `User=root`. Until that landed every `fs.create` died at `mkfs.xfs: error - cannot set blocksize 4096 on block device /dev/xi_ss: Permission denied`, and `fs.grow` carried the same defect unexercised. See ADR-0007 §Sandbox.
...
- **`systemctl is-enabled` vs ActiveState** — RESOLVED. The probe queries BOTH `systemctl is-enabled` (→ the boolean `mount_unit_enabled`) and `systemctl is-active` (→ `mount_unit_state`, one of the schema enum `active|inactive|failed|activating|deactivating`). The earlier code wrote the `is-enabled` STRING (`enabled`/`disabled`/`static`/…) into `mount_unit_state`; that is not a member of the ActiveState enum, so on a real host every enabled managed `.mount` unit 400'd at `/internal/v1/observed` and was silently dropped by the publisher's non-retryable-4xx path — surfacing as "No XFS filesystems found" on a freshly-installed node while the FS was mounted and active. `mount_unit_state` is now OMITTED when `is-active` reports a non-enum value (e.g. `unknown` for a masked/not-found unit), since ingest strips `required`. The enrichment's mountinfo cross-ref remains the authoritative `mounted`. This was invisible to CI because inbound schema validation is skipped in the unit context and the contract fixture hand-codes a valid `mount_unit_state: "active"`; the regression is now pinned by a probe-output-vs-compiled-schema test (`agent/probe/filesystem.test.ts`) and an ingest accept/reject pair (`api/internal-observed-schema.test.ts`), and the publisher now surfaces 4xx rejections via `onPublishError` instead of dropping them silently.
- **Which mount table the agent reads** — RESOLVED (2026-08-16, found on hardware). The probe read `/proc/self/mountinfo`, which is the mount table of the *reading process's mount namespace*. The agent unit runs `ProtectSystem=strict` + `ProtectHome=yes` + `PrivateTmp=yes` — any one of which puts the service in a namespace of its own — so the agent was reporting **its own sandbox's view** of every mount. Captured on the reference node, same device (`259:93`), same instant: the agent's namespace (`mnt:[4026548149]`) shows `/mnt/data` as `ro,nosuid,noatime,nodiratime` while the host's (`mnt:[4026531841]`) shows `rw,noatime,nodiratime`. `ProtectSystem=strict` makes the hierarchy read-only for the service outside its `ReadWritePaths`, and `/mnt/data` is not one. Published as `effective_mount_options`, that told the Filesystem screen — and every other client — that the data filesystem was read-only, while NFS exported it `rw` and clients wrote to it. All three agent readers now use `HOST_MOUNTINFO_PATH` (`/proc/1/mountinfo`; PID 1 is by definition in the root namespace), in [agent/fs/mountinfo-source.ts](../../xiNAS-MCP/src/agent/fs/mountinfo-source.ts), with **no fallback** to `/proc/self` — falling back would restore the wrong-but-plausible reading the constant exists to remove, and a failed sweep is already handled (boot skips reconcile, `PollDriver` retries) while a silently wrong one is not.

  Two things this did **not** break, both worth recording because they were checked rather than assumed. The xiRAID delete guard ([task/wiring.ts](../../xiNAS-MCP/src/agent/task/wiring.ts)) matches on `source` and on `logdev=`/`rtdev=` in the fs-specific **super** options — properties of the superblock, not of the mount, so they read identically in both namespaces; and the mount is visible in the agent's namespace as a slave (`master:76`), so propagation keeps new mounts appearing. The guard was therefore correct throughout, and is switched to the host table only for consistency. Nor was the state store stale: the api deliberately skips content-identical upserts (`skippedUnchanged` in [internal/observed.ts](../../xiNAS-MCP/src/api/internal/observed.ts)), so the row sitting at `revision: 1` since install meant *the agent kept publishing the same wrong value*, not that it had stopped publishing.
