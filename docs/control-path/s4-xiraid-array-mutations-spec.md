...
- **ADR-0006** — writability matrix (topology immutable; spares + tuning live), spare-pool lifecycle (`spec.spare_pool` references an existing pool by name; no executor-owned pool), per-operation contracts, the engine-enforced dangerous gate, `rollback_model` enums.
...
2. `validateModifySpec`: tuning ranges (§Tuning ranges below); when `spare_pool` names a pool, it must exist in observed state and hold at least one drive (`spare_pool_not_found` / `spare_pool_empty`). No disk-level checks — the pool's drives are the pool surface's business.
3. `affected_resources = [ XiraidArray#id (primary, first), the target Pool, the currently-attached Pool ]` — the array providers have no separate `lease_resources`, so this doubles as the lease set. Leasing the pool BY NAME serializes an attach against a concurrent Spare Pools mutation on it, which the retired disk leases never did; leasing the pool being **left behind** does the same for a detach or a re-point.
...
1. A detach whose only payload is `name` + `sparepool: ''` — exactly what
   `apply_spares` would send for a detach if it used `''` and no tuning changed —
...
live sparepool is non-empty — `pre?.arraySparepool !== ''`; `rollback` only
calls `raid_modify` when live ≠ captured).

`rollback`: restores the array's previous sparepool name (`raid_modify { sparepool: <prior-name-or-'null'> }`), computed from the executor's own in-process pre-state capture — `preflight` reads live `raid_show`/`pool_show` and stashes `{ arraySparepool, targetPoolActive }` in a `WeakMap` keyed by the run's spec object (item 6 above), not from any `enriched_spec.current_*` field — and deactivates the pool **only if this run activated it**, tracked via `ctx.stash.pool_activated` (set by `apply_spares` the moment `pool_activate` succeeds, read back by `rollback`). The deactivation is attempted **first**, before the sparepool linkage is restored: `apply_spares` can activate the pool and then fail on the very next call, so rollback must undo the activation even when there is otherwise nothing else to restore — attempted non-fatally, with a warning and a retry after the linkage restore, so a rejected `pool_deactivate` cannot itself block the rest of rollback (see `xiraid-array-executor.ts` rollback for the exact sequencing). No `pool_create`/`pool_add`/`pool_remove`/`pool_delete` ever runs in rollback, mirroring `apply_spares` itself — the pool is never the executor's to build or destroy. Tuning needs no rollback by construction (it is the last stage and atomic). A rollback failure → `requires_manual_recovery` (S2 runner).

**Apply-time re-check compatibility.** The route's §8 re-check runs the provider preflight on the **persisted (enriched) spec**, so `parseModifySpec` is **tolerant of the enrichment keys**: it narrows `spare_pool`/`tuning` and *ignores* unknown keys (`id`, `device_by_id`) rather than whitelisting. The **route** owns rejection of every non-writable key, checking the **raw PATCH body** for topology keys, create-only tuning keys and `spare_disk_ids` before any plan, and it produces the per-field `UNSUPPORTED` 422. Provider preflight is idempotent over its own enriched output (`{ id, ...change }`, which never carries `spare_disk_ids`).

`spare_disk_ids` is the **one** key `parseModifySpec` also rejects, and the reason is not redundancy with the route. Plan rows have no TTL, so a modify planned **before** 2026-08-29 persists an enriched spec of the old shape (`{ id, spare_disk_ids, device_by_id }`); applied after the upgrade, a tolerant parse would drop the key, `apply_spares` would emit `skipped (no spare_pool change)` and the task would land as a **success that attached nothing** — the silent skip the route's 422 exists to prevent, on the one path the route cannot see (its gate runs against the raw body at plan time only). On the plan path the route's nicer 422 still fires first.

**Create-with-spares un-deferral.** `validateCreateSpec` drops `spare_pool_deferred`; when `spec.spare_pool` is set, preflight checks the named pool exists and is non-empty on the daemon (a pool reference, not a set of disks to provision — no member-disk checks apply to it); `translate.toRaidCreateRequest` passes `spec.spare_pool` straight through as `sparepool`; the **create executor** issues **`pool_activate`** inside `create` only when the named pool reads inactive (no `pool_create`) before `raid_create`, and its `rollback` deactivates the pool again only if this run activated it (no `pool_delete` — the pool is not the executor's to delete).

That deactivation is the **first** thing the create rollback does — above *both* of its early returns (unparsable spec, `create` never attempted), not after the array destroy. `create` activates the pool and *then* calls `raid_create`, so a `raid_create` that rejects, or a spec that no longer parses at rollback time, must still leave the operator's pool disarmed; an undo placed after the destroy is skipped on exactly those paths. The first attempt is **non-fatal**: it is not established whether the daemon accepts `pool deactivate` while an array still references the pool (neither the 4.4 command reference nor `xiNAS-MCP/xiraid-analysis/api_behavior_doc.md` settles it, and the fake transport permits it unconditionally), and a rejection propagating out of rollback would abandon the `raid_destroy` that follows, ending the task in `requires_manual_recovery` with this run's array still on the daemon. So a failure there is caught, emitted as a warning naming the pool and the error, and **retried after the array destroy**; the retry's failure is never swallowed (→ `rollback_failed` → `requires_manual_recovery`). The modify rollback uses the identical sequence around its linkage restore.
...
- `preflight` — two live host-level guards, closing the route-recheck→destroy TOCTOU window the api cannot close (the executor has only `spec`, no KV — but it runs as root on the host):
  ...
  2. **the volume is not in use by any mount**: an injected `readMounts()` (real impl: read `/proc/self/mountinfo` + the existing `lib/parse/mountinfo.ts`) must show no mount that (a) has `/dev/xi_<name>` as its **source device**, or (b) **references it as an external device** through a `logdev=/dev/xi_<name>` or `rtdev=/dev/xi_<name>` mount option, in either the per-mount VFS options or the filesystem-specific super options. Case (b) is not hypothetical on xiNAS: the standard layout puts the XFS journal on a separate RAID 10 array, and such a filesystem never names that array as its mount source — destroying it under the mounted filesystem corrupts the journal. A mount that appeared after the route re-check fails the preflight here. Active NFS sessions on a dependent share require that share's filesystem to be mounted, so the mount guard subsumes the session race for data safety; the api-side blockers remain the richer, user-facing signal.
- `destroy` — `raid_destroy { name, force: true }`. **`raid_destroy` is the only step in this stage that may fail the task**: once it returns success the array is destroyed (irreversible, the intended outcome), so everything after it is **best-effort** and MUST NOT throw out of the stage (a throw here triggers `rollback`, and since the array is now gone the rollback escalates to `requires_manual_recovery` — telling the operator the destroy half-failed when it succeeded). **Delete does not touch the array's spare pool at all.** There is no cleanup step here: the pool the array referenced (if any) is not the executor's to deactivate or delete — it belongs to the pool surface, outlives the array, stays active, and remains available to attach to another array or to be managed from Spare Pools.
- `verify` — **bounded confirmation, never escalates a completed destroy.** `raid_destroy` already returned success, so verify only confirms the array cleared: it polls `raid_show` until the array is gone (a short bounded wait absorbs async propagation on daemons where `raid_destroy` is not instantaneous). A **transient `raid_show` error** is tolerated — logged as a warning and accepted (the array was acknowledged destroyed; the observe path resurfaces the true state). Only an array that is **still present after the wait** fails the stage (a genuine "destroy did not take"), and it throws into `rollback`, which sees the array still present → **no-op → clean `failed (FAILED_PARTIAL_ROLLED_BACK)`, retryable — never `requires_manual_recovery`**.

**`rollback()` is gated on attempted destruction, then live-state decided** (NOT an unconditional throw). The `destroy` stage records a `destroy_attempted` marker in the per-run executor stash (`ctx.stash`) immediately **before** issuing `raid_destroy`; rollback reads it:

- **Marker absent — the failure happened in `preflight`** (array not in live `raid_show`: stale observed state / repeat delete; mounted volume; or the daemon unreachable so preflight's `raid_show` itself threw). Nothing destructive was attempted → rollback is a **no-op that does not query the daemon** (a `raid_show` here could throw on an unreachable daemon and falsely escalate) → terminal `failed (FAILED_PARTIAL_ROLLED_BACK)`, a clean retryable failure — never `requires_manual_recovery`.
- **Marker present — `raid_destroy` was issued.** Live-state decision: if the array **still exists** in `raid_show`, the destroy did not take (rejected call, or a verify that found the array still present) → **no-op** → clean `failed (FAILED_PARTIAL_ROLLED_BACK)`, retryable. If the array is **gone or `raid_show` is unreachable**, the destroy started and its effects are unknowable → **throw** (`'destructive operation: rollback unsupported'`) → `rollback_failed` → terminal `requires_manual_recovery (FAILED_MANUAL_RECOVERY_REQUIRED)`.

Because the post-`raid_destroy` steps above never throw, `requires_manual_recovery` is reserved for a `raid_destroy` call that failed mid-way with genuinely unknowable state — ADR-0006's "no rollback for a destructive op" applies to *attempted destruction*, not to a preflight that touched nothing, and not to post-destroy bookkeeping (the verify confirmation) after the array is already gone.
...
- Failure hooks stay name-deterministic: any mutating verb against a name ending `-fail` rejects (the S3 `roll-fail` pattern, extended to every new verb); additionally a `raidModify` carrying **tuning keys** (any field beyond `name`/`sparepool`) against a name ending `-fail-tuning` rejects — this targets the tuning stage specifically, leaving `apply_spares` to succeed first. A plain `-fail` **array** name cannot do that: the hook keys on each request's own name, and `apply_spares` issues a `raid_modify` carrying the array's name, so it would reject one stage too early. Pool verbs are unaffected either way — they hook on the *pool's* name, which is independent of the array's.

## 10. Error model — reuse existing codes (no additions)

New **blocker codes** (plan `blockers[]`, not `ErrorCode`s): `dangerous_flag_required`, `dependent_filesystem_mounted`, `dependent_share_active`, `uuid_invalid`. A named `spare_pool` that is absent from the daemon or has no drives surfaces as an **executor preflight failure**, not a plan blocker — the api cannot see live pool state (there is no foreign-pool concept: any pool the operator names is a valid target). New `details.reason` discriminators on `PRECONDITION_FAILED`: `dangerous_flag_required` (engine), `observed_revision_stale` (route). Topology writes on PATCH → `UNSUPPORTED` (422, `reason: 'topology_immutable'`); create-surface tuning keys (`resync_enabled`/`discard`/`drive_trim`) on PATCH → `UNSUPPORTED` (422, `reason: 'create_only_tuning'`). Everything else unchanged from S3/S2.
...
- **Unit:** engine dangerous gate (destructive+flag absent → 412 `dangerous_flag_required`, nothing written; flag true → proceeds; non-destructive ignores); `validateModifySpec` (rejection of non-writable keys is route-level — validator covers tuning ranges + the named-pool rules `spare_pool_not_found`/`spare_pool_empty`; the parser tolerates enrichment keys so the apply re-check accepts its own plan); the route's `spare_disk_ids` 422 (`reason: 'observed_only'`, no plan row written); modify/import/delete providers (blockers, `affected_resources` ordering + Pool leasing on both the target and the currently-attached pool + dependent leasing, enriched_spec contents, the **status-shaped** Filesystem dep walk); the three executors over the in-memory fake (modify: attach-with-activation / attach-an-already-active-pool / detach-without-deactivation + missing-or-empty target pool at preflight + tuning-last ordering + rollback restoring the prior name, empty **and** non-empty; import: adopt/unknown-uuid/un-adopt rollback; delete: destroy leaves the pool untouched / **mounted-volume preflight guard** / preflight failures — vanished array, daemon unreachable — → clean `failed` via no-op rollback (destroy never attempted, no daemon query) / attempted destroy leaving the array gone → rollback-throws → `requires_manual_recovery`); create-with-spares (a missing or empty named pool fails preflight; `pool_activate` before `raid_create` only when the pool reads inactive, and no `pool_create`; rollback deactivates first — including when the first attempt is rejected, which must not abandon the array destroy — and never deletes the pool); fake-transport pool-activation/import/`config_only`/`-fail-tuning` behaviors.
...
- **e2e** (extends the S3 suite's fixture pattern): **modify** — attach an operator-created pool to an array that has none (the 2026-08-29 field regression: the attach must succeed and must not leave a second pool behind) + set tuning → success → observed `status.spare_pool` updated; detach with `spare_pool: null` → the pool survives; `spec.spare_disk_ids` on `PATCH` → 422; **import** — seed an `import_candidate`, plan+apply adopt → success → array observable; **delete** — apply without `dangerous` → 412; with a seeded dependent mounted filesystem → 412 `dependent_filesystem_mounted`; clean array + `dangerous: true` → success → gone from `GET /arrays`; `-fail` hooks exercise one rollback path.
...
| **T4** | Create-with-spares un-deferral: create provider (resolve the named pool) + create executor `pool_activate` step + rollback deactivation. TDD. |
| **T5** | Modify provider + `PATCH /arrays/:id` route (non-writable-key 422 pre-plan; §4 binding; §8 re-check). TDD. |
| **T6** | Modify executor (attach/detach a named pool, tuning last, rollback restores the prior linkage). TDD. |
...
| **T10** | Delete executor (destroy, the referenced pool left untouched, rollback-throws → manual recovery). TDD. |
...
- **Pool-name length bound** — resolved: the control path no longer derives a pool name from an array's, so no combined length can overflow. The pool `NAME_RE` keeps a 64-char bound as a deliberate choice; xiRAID documents no pool-name length.
