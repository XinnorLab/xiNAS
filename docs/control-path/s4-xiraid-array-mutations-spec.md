...
**`rollback()` is live-state decided** (NOT an unconditional throw): if the array **still exists** in `raid_show`, nothing destructive happened (a `preflight` failure — busy volume, vanished-then-reappeared race — or a verify that found the array still present) → rollback is a **no-op** → terminal `failed (FAILED_PARTIAL_ROLLED_BACK)`, a clean retryable failure. If the array is **gone or `raid_show` is unreachable**, the destroy started and its effects are unknowable → **throw** (`'destructive operation: rollback unsupported'`) → `rollback_failed` → terminal `requires_manual_recovery (FAILED_MANUAL_RECOVERY_REQUIRED)`. Because the post-`raid_destroy` steps above no longer throw, `rollback` is now reached **only when `raid_destroy` itself fails** (or a preflight guard fires), so `requires_manual_recovery` is reserved for a destroy call that failed mid-way with genuinely unknowable state — ADR-0006's "no rollback for a destructive op" applies to *attempted destruction*, not to post-destroy bookkeeping (pool cleanup / confirmation) after the array is already gone.
