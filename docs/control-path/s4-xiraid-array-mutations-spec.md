...
**Apply-time re-check compatibility.** The route's §8 re-check runs the provider preflight on the **persisted (enriched) spec**, so `parseModifySpec` is **tolerant**: it narrows `spare_pool`/`tuning` and *ignores* unknown keys (`id`, `device_by_id` — the enrichment) rather than whitelisting. The **route** owns rejection of every non-writable key, checking the **raw PATCH body** for topology keys, create-only tuning keys and `spare_disk_ids` before any plan (the parser never sees a raw body with them on the plan path, and the enriched spec never contains them). Provider preflight is idempotent over its own enriched output.
