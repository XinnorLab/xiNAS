...
- `destroy` — `raid_destroy { name, force: true }`. **`raid_destroy` is the only step in this stage that may fail the task**: once it returns success the array is destroyed (irreversible, the intended outcome), so everything after it is **best-effort** and MUST NOT throw out of the stage (a throw here triggers `rollback`, and since the array is now gone the rollback escalates to `requires_manual_recovery` — telling the operator the destroy half-failed when it succeeded). Accordingly, the spare-pool cleanup (`pool_show`, then `pool_deactivate` + `pool_delete` when an `xnsp_<id>` pool exists) is wrapped best-effort: any failure is logged as a warning ("array destroyed; remove `xnsp_<id>` manually if it lingers") and the stage still succeeds.
