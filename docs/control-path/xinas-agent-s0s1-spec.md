...
| `fs.create`, `fs.mount`, `fs.unmount`, `fs.grow`, `fs.set_quota_mode` | **Superseded by the task envelope (S5, ADR-0007)** | Mutations dispatch via `task.begin` + the executor registry; these names left the enumerated RPC surface in S5 and now return `-32601` like any unknown method (no longer a contract violation for these names). |
...
| `health.probe` | **Real (S7, ADR-0009; schema 2 since S19a)** | Read-style diagnostic: `params { level: 'standard'\|'deep', desired_nfs_profile?, first_export_path? }` → `{ schema: 2, sections }` — one typed Section (`status: success\|error\|timeout\|permission_denied\|not_supported`, `observed_at`, `value`/`error`) per source: parsed license (NEVER the raw xicli blob), fresh rdma links (a refused or failed `rdma` call is a typed failure, never an empty list), collector health, the helper's dry-render checksums; `deep` adds the `probes` section (per-run fs_io + PID1 loopback probes on the hardened host, each row with a cleanup verdict). Idempotent; mutates nothing beyond self-cleaned probe artifacts. |
| `health.baseline` | **Planned (S19c, ADR-0018 — design, not yet implemented)** | `params { profile_path, timeout_s }` → runs the Python engine `python3 -m xinas_menu.health <profile> <log_dir> --json --no-save` as a capped, read-only subprocess (profile path realpath-checked against the profiles dir, sanitized env, own process group, SIGKILL at the cap) and returns `{ status, collected_at, engine, report, error }`. One run at a time per agent; concurrent callers share the result. `s19-mcp-health-prompt-spec.md` §8.3. |
| `health.probe.run` | **Real (S19a, ADR-0018)** | `params { probe: 'fs_io'\|'nfs_loopback', path, run_id?, timeout_ms? }` (the api resolves ids to paths; `timeout_ms` 1 000–60 000, default 20 000) → `ProbeOutcome & { probe, path }`: one hardened active probe — a per-run unique file under a checked `.xinas-health` directory (`O_EXCL`, `O_NOFOLLOW`, same-device fstat) or a per-run loopback mountpoint behind a lock; `cleanup` reports what happened to the artifact, the timeout is enforced on the agent. One probe in flight per node (any kind, deep included; the ProbeHost is the admission point): a concurrent call is `-32000` with `data.code: PROBE_IN_PROGRESS`. `health.probe level=deep` runs on the same host. `s19-mcp-health-prompt-spec.md` §9.2–§9.3. |
| `health.baseline` | **Real (S19c, ADR-0018)** | `params { profile_path, timeout_s: 1..900 }` → the Python baseline engine (`<health_baseline.python> -m xinas_menu.health <profile> <log_dir> --json --no-save`) as a capped, sandboxed, read-only subprocess: `profile_path` MUST realpath-resolve inside `health_baseline.profiles_dir` (refused before spawn otherwise), cwd `module_root`, env only `PATH`/`LANG`/`PYTHONPATH`, stdin closed, stdout ≤ 4 MiB, stderr ≤ 64 KiB, own process group SIGKILLed at the deadline (less a 250 ms grace, so the engine's own typed `timeout` reaches the caller) → `{ status: success\|error\|timeout\|not_supported, collected_at, duration_ms, engine: { module, version }, report: <engine JSON verbatim> \| null, stderr_tail, error? }`. `params { sections: true, timeout_s? }` → `{ status, collected_at, sections: string[] \| null, version }` (the engine's `--sections`, cached after the first success). One engine subprocess at a time; concurrent callers of the same profile share the run. Concurrency (amended 2026-09-10, validation F09): the deadline is absolute from the moment the RPC arrives (`now + timeout_s`), not from spawn; a queued run whose deadline passes before its turn is `timeout` without a spawn; callers are coalesced by the profile's realpath across the WHOLE queue (A, B, A spawns A once), each joiner keeping its own deadline; at most four distinct profiles wait (`QUEUE_FULL` beyond that); the `--sections` call queues under the same rules. Mutates nothing (`--no-save`). `s19-mcp-health-prompt-spec.md` §8.3, §12.2. |
...
3. **Single SQLite transaction** opens. For each delta: `tx.put` (upsert) or `tx.delete` (delete). **Sweep dedupe (skip-unchanged):** before each upsert the handler compares the incoming value against the stored one with the `observed_at` stamps stripped (top-level and `status.observed_at`, canonical-JSON compare); an unchanged value is SKIPPED — no write, no revision bump — and counted in `skipped_unchanged`. Rationale: `PollDriver` full-sweeps every collector on its interval and collectors re-stamp `observed_at` each sweep; without the dedupe every sweep bumped every observed revision, so revision-pinned freshness (the S4/S5 route bindings, `observed_freshness_ref`) only held within one sweep window (~30 s) on live hosts. With it, observed revisions move only on CONTENT change — which is what the pins mean. Consequence: a stored row's `observed_at` records when its current content was last written, not the latest sweep; system-level liveness is the heartbeat tracker's `agent_last_observed_push` (step 5), which fires on every push regardless. Skipped rows still count as "present in the batch" for reconcile (next sentence). For each kind in `complete_snapshots`: after applying its deltas, `tx.list({ prefix: '/xinas/v1/observed/<Kind>/' })` to enumerate current keys at the transaction's snapshot, compute the set difference (current − batch-upserts), and `tx.delete` each leftover.
...
6. Response: `200 OK` with envelope body `{ accepted: N, deleted_by_reconcile: M, skipped_unchanged: K, state_revision: R }`.

**Audit:** the observation push is **not** audited. The audit middleware skips `POST /internal/v1/observed` (alongside the `/mcp` transport frame) because it is high-frequency telemetry ingest, not an operator operation: `PollDriver` full-sweeps every collector on its interval, so auditing the push would flood the trail with hundreds of identical `http.POST./observed` rows that bury real actions (`health.check`, `share.create`, …). The observed-state writes it performs remain visible via the `GET` observed-state routes. The agent's other internal write routes (`/internal/v1/agent_started`, `/internal/v1/task_progress`) are low-frequency and stay audited.
...
Observation POSTs are **not** audited. The `auditMiddleware` skips `POST /internal/v1/observed` at the top of the middleware (the same place the `/mcp` transport frame is skipped), before it registers the `res.on('finish')` hook. Rationale: the push is high-frequency telemetry ingest, not an operator operation — `PollDriver` full-sweeps every collector on its interval, so one audit row per push floods the trail with hundreds of identical `http.POST./observed` rows and buries the real actions operators come to the audit trail for (`health.check`, `share.create`, RAID, network, …). What was pushed and when is instead observable through the observed-state `GET` routes (each carries `status.observed_at`) and the agent's `agent.health` (`last_publish_error`, heartbeat/`agent_last_observed_push`); it does not belong in the operator audit trail.

The exclusion is scoped to the observation push only. The agent's other internal write routes — `POST /internal/v1/agent_started` and `POST /internal/v1/task_progress` — are low-frequency and remain audited via the ordinary best-effort middleware path (`res.on('finish')` → `state.audit.queue()`), with `principal: "agent:root"`, `client_type: "rest"`, and the standard `kind`/`parameters_hash`/`result_hash`/`request_id` fields.
