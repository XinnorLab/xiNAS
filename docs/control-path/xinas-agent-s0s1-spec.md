...
| `health.baseline` | **Real (S19c, ADR-0018)** | `params { profile_path, timeout_s: 1..900 }` → the Python baseline engine (`<health_baseline.python> -m xinas_menu.health <profile> <log_dir> --json --no-save`) as a capped, sandboxed, read-only subprocess: `profile_path` MUST realpath-resolve inside `health_baseline.profiles_dir` (refused before spawn otherwise), cwd `module_root`, env only `PATH`/`LANG`/`PYTHONPATH`, stdin closed, stdout ≤ 4 MiB, stderr ≤ 64 KiB, own process group SIGKILLed at the deadline → `{ status: success\|error\|timeout\|not_supported, collected_at, duration_ms, engine: { module, version }, report: <engine JSON verbatim> \| null, stderr_tail, error? }`. `params { sections: true, timeout_s? }` → `{ status, collected_at, sections: string[] \| null, version }` (the engine's `--sections`, cached after the first success). One engine subprocess at a time; concurrent callers of the same profile share the run. Mutates nothing (`--no-save`). `s19-mcp-health-prompt-spec.md` §8.3, §12.2. |
