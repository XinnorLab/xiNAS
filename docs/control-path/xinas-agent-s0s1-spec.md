...
- Generated by the `xinas_api` role's bootstrap step (`uuidgen > /var/lib/xinas/controller-id`, guarded by `creates:` so re-runs preserve the value). Preserved across reboot + upgrade.
