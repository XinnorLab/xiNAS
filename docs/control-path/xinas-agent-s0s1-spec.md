...
| `health.baseline` | **Planned (S19, ADR-0018 — design, not yet implemented)** | `params { profile_path, timeout_s }` → runs the Python engine `python3 -m xinas_menu.health <profile> <log_dir> --json --no-save` as a capped, read-only subprocess (profile path realpath-checked against the profiles dir, sanitized env, own process group, SIGKILL at the cap) and returns `{ status, collected_at, engine, report, error }`. One run at a time per agent; concurrent callers share the result. `s19-mcp-health-prompt-spec.md` §8.3. |
| `health.probe.run` | **Planned (S19, ADR-0018 — design, not yet implemented)** | `params { probe: 'fs_io'\|'nfs_loopback', target, run_id?, timeout_s }` → one hardened active probe: per-run unique artifact under a root-owned `.xinas-health` directory (`O_EXCL`, `O_NOFOLLOW`, same-device check) or a per-run loopback mountpoint serialized by a lock; cleanup failure is reported, the timeout is enforced on the agent. `health.probe level=deep` is re-pointed at the same host. `s19-mcp-health-prompt-spec.md` §9.3. |
