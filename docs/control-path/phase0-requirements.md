...
**Requirement:** At the same bootstrap point, `xinas-api` must also adopt the installer's declared NFS exports into desired state, once. `seedShares()` (`xiNAS-MCP/src/api/seed-shares.ts`) runs immediately after the ADR-0016 singleton seed and reads the manifest the `exports` role renders (`/var/lib/xinas/seed/shares.json`), writing one desired `Share` per entry the first time it finds the manifest with the one-time marker `/xinas/v1/meta/shares_seeded` unset. Once set, the marker is never cleared by the api, so an operator who later deletes a seeded share is not resurrected on restart. See [docs/Installer/fs-exports-spec.md §2.6](../Installer/fs-exports-spec.md#26-install-time-share-adoption-seed-manifest) for the manifest contract and [the design doc](../superpowers/specs/2026-07-03-nfs-share-seed-adoption-design.md) for the full rationale.
**How to verify:** On a fresh install, confirm `/xinas/v1/desired/Share/<id>` exists for the preset's default export and `GET /api/v1/shares` returns it without any wizard-created share; delete it via the TUI, restart `xinas-api`, and confirm it stays deleted.
