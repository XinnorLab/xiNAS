...
- `terminal` sets the terminal state + `result_hash` + `snapshot_after`, **releases leases**, and frees the worker slot. The terminal `tasks` transition, the desired-snapshot capture (terminal-success), the lease release, and the intent revert (non-success) run inside **one `db.transaction`** so a crash mid-handler cannot leave a lease orphaned on a terminal task (they commit together or not at all; a failed apply re-runs cleanly since `last_event_sequence` did not advance).
...
**Periodic lease sweep (ADR-0004 "periodic sweep").** Separately from `reconcile()`, the api runs a lightweight timer that calls **only** `LeaseManager.reapExpiredTerminalLeases()` every `LEASE_SWEEP_INTERVAL_MS` (default **30 s** = TTL/2; the lease TTL is 60 s). This is what ADR-0004 §Stale-lease-recovery means by "on `xinas-api` startup AND on a periodic sweep": without it, a lease that outlives its **terminal** task (a release that didn't run — e.g. the crash window in §6's terminal handler, closed separately by the transaction there) is only reclaimed at the next api restart or agent-reconnect edge — turning the 60 s TTL into "locked until restart," which surfaces to operators as a spurious `CONFLICT: resource is locked by another task` on the *next* mutation of that resource (e.g. deleting a just-created share). The timer is `unref()`ed (never keeps the process alive) and stopped on server close.

The timer reaps **only leases whose holder is already terminal** (or whose task row is gone) — it deliberately does NOT flip a `queued`/`running` task to `requires_manual_recovery` the way the full `sweepExpired()` does. A frequent timer cannot distinguish "the agent died" from "this stage legitimately runs longer than the lease TTL without emitting a heartbeat-bearing progress event" (the runner bumps `heartbeat_at` per progress event, and a long executor stage may push none until it completes). So non-terminal-task recovery stays with the startup/reconnect `reconcile()` passes, where an api restart genuinely means the executor handle was lost (ADR-0004's own rationale). Re-dispatch/adopt also stays timer-free (§5.3); redispatch remains startup + reconnect driven.
