...
- `risk_level`: `non_disruptive` (`force: false`) / **`destructive`** (`force: true`). `rollback_model: unsupported` **always** (amended 2026-09-04, S16 §9.1 — this line used to say `non_disruptive` for a non-force create, contradicting the rollback contract two bullets down: a completed `mkfs` is never reverted, whether or not the device carried a filesystem before). The non-force `risk_level` stays `non_disruptive` only because `preflight` re-proves live that nothing is being destroyed. Over MCP, `rollback_model: unsupported` routes every create through S15's out-of-band (URL) confirmation.
- **Point of no return** (S16 §9.2): the `mkfs` stage. A cancel that arrives after `mkfs` started is refused by the executor (`cancel_refused_reason: irreversible_stage_started`); the task completes or fails on its own and is never reported `cancelled`.
