...
2. **Destruction gate** (`TaskFailed` whose `error_message` is the fs-executor preflight gate — the device `already carries a … filesystem`, [xiNAS-MCP/src/agent/task/fs-executor.ts](../../xiNAS-MCP/src/agent/task/fs-executor.ts)): the screen offers the **force-recreate consent** — a Yes/No dialog quoting the task's failure detail and warning that retrying with `force: true` DESTROYS the existing data on the device. On Yes it re-submits the same spec with `force: true` and `dangerous=True`. This is the *only* failure that offers the retry.
   ...
   **The retry runs under the same surface as the first attempt** — its own `TaskWaitDialog` showing task-state progress, and the same cancel offer. Both submissions therefore go through one helper rather than two call sites; a retry that reports progress differently from the attempt it repeats is a defect, not a variation. This is worth stating because the failure mode is silent and asymmetric: the force retry runs `mkfs` over an existing filesystem, so it is both the slower of the two attempts and the destructive one, and it is precisely the one an operator must be able to watch and abort. The screen showed a toast and a static line here until 2026-08-16.
