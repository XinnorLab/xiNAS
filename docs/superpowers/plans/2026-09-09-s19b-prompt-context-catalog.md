...
- [ ] **Step 3: Implement** in `config.ts` next to the subscriptions block: the interfaces, `HEALTH_PROMPT_DEFAULTS`, and `resolveHealthPromptConfig` with a small `range(key, value, min, max)` helper that throws `new Error(\`mcp.health_prompt.${key}: must be an integer between ${min} and ${max}\`)`; `probe_policy_max` must be one of the two enum values; `policy_version` must match `/^[A-Za-z0-9.+-]{1,32}$/`; `profiles_dir` must start with `/`; `template_path` when set must start with `/`. Ranges: analysis_seconds 30–3600, tool_calls 5–500, roles 1–8, active_probes_per_node exactly 1, probes_per_run 0–16, retries 0–5, run_ttl_seconds 300–7200, timeout_s.* 10–900, max_age_s_default 0–3600.
