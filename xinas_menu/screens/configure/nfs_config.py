...
def _find_nfs_exports_file(preset: str = "default") -> Path | None:
    for base in _PRESETS_DIR_CANDIDATES:
        p = base / preset / "nfs_exports.yml"
        if p.exists():
            return p
    return None
...
class NFSConfigScreen(Screen[bool]):
    ...
    def __init__(self, preset: str = "default", **kwargs) -> None:
        ...
        self._preset = preset
        self._cfg_path = _find_nfs_exports_file(preset)

    def compose(self) -> ComposeResult:
        title = str(self._cfg_path) if self._cfg_path else "(no NFS exports config found)"
        yield Label(f"  ── NFS Exports Config — {title} ──")
        content = ""
        if self._cfg_path and self._cfg_path.exists():
            content = self._cfg_path.read_text()
        yield TextArea(content, id="nfs-config-editor", language="yaml")
