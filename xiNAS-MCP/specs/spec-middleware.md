...
| `admin` | operator + `raid.create/modify_performance/unload/restore/delete`, `pool.create/delete/add_drives/remove_drives/acquire`, `disk.secure_erase`, `network.configure`, `auth.validate_kerberos`, `auth.create_user`, `auth.delete_user`, `auth.change_password`, `auth.set_user_lock`, `auth.change_shell`, `auth.add_to_group`, `auth.remove_from_group`, `config.rollback` |
