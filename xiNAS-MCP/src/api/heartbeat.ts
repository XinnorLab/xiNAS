...
/**
 * Build the production `healthProbe` for a HeartbeatTracker: a thin wrapper
 * over the shared `createAgentRpcClient` JSON-RPC-2.0-over-NDJSON client
 * (`agent-client.ts`). Each call performs one `agent.health` round-trip and
 *   - resolves `{ version, collectors }` from the JSON-RPC `result`, or
 *   - rejects on connect error (ECONNREFUSED/ENOENT), a JSON-RPC `error`,
 ...
 * other reject → a plain heartbeat failure.
 *
...
export function createAgentHealthProbe(agentSocketPath: string): () => Promise<AgentHealthResult> {
  const client = createAgentRpcClient(agentSocketPath);
  return async () => {
    const result = await client.call('agent.health', {}, HEALTH_PROBE_TIMEOUT_MS);
    // Map the JSON-RPC `result` (version + collectors) through. A null/absent
    // result maps to {}. Passing the object straight avoids materializing
    // explicit `undefined` properties, which exactOptionalPropertyTypes rejects.
    return (result as AgentHealthResult | null | undefined) ?? {};
  };
}
