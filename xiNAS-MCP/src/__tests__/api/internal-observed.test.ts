...
> {
  ...
  setup.config.tokens[AGENT_TOKEN] = {
...
describe("POST /internal/v1/observed", () => {
  ...
  it("reconciles: deletes keys under prefix not in the batch when complete_snapshots includes the kind", async () => {
    ...
    const stored = setup.state.kv.get<{ name: string }>(
      "/xinas/v1/observed/Disk/nvme0n1",
    );
...
describe("observed sweep dedupe (skip-unchanged)", () => {
  ...
  const DISK = (observedAt: string, model = "T") => ({
    ...
    value: {
      kind: "Disk",
      id: "nvme0n1",
      status: { model, observed_at: observedAt },
    },
  });
  ...
  it("complete-snapshot reconcile keeps skipped-unchanged rows (not deleted as absent)", async () => {
    ...
    expect(
      setup.state.kv.get("/xinas/v1/observed/Disk/nvme0n1"),
    ).not.toBeNull();
  });
    ...
    const res = await push([
      {
        ...
        value: {
          kind: "Disk",
          id: "nvme0n1",
          status: { b: 2, a: 1, observed_at: "y" },
        },
      },
    ]);
    expect(res.body.result.skipped_unchanged).toBe(1);
    expect(
      setup.state.kv.get("/xinas/v1/observed/Disk/nvme0n1")?.revision,
    ).toBe(rev1);
  });
  ...
  // ADR-0015, S12 T6: GC must NOT fire on non-ConfigSnapshot pushes.
  // A freshly-captured snapshot-desired payload has no observed ConfigSnapshot
  // row yet (the 60s collector hasn't fired). An unrelated event-driven push
  // (Disk, Network…) in that lag window must leave the payload intact.
  it("snapshot-desired payload survives a non-ConfigSnapshot push but is pruned on a complete ConfigSnapshot re-emit", async () => {
    const SNAP_ID = "snap-race-001";
    const desiredKey = `/xinas/v1/snapshot-desired/${SNAP_ID}`;

    // Seed a snapshot-desired payload as if capture just wrote it.
    // There is NO matching observed ConfigSnapshot row yet (lag window).
    setup.state.kv.put(desiredKey, { snapshot_id: SNAP_ID, kinds: {} });

    // Push a Disk delta — this is the event-driven push that arrives in the
    // lag window. ConfigSnapshot is NOT in complete_snapshots.
    await push(
      [
        {
          kind: "Disk",
    ...
    // The payload must survive — GC must NOT have fired.
    expect(setup.state.kv.get(desiredKey)).not.toBeNull();

    // Now do a complete ConfigSnapshot re-emit with no matching row for SNAP_ID.
    // (The lag window is still open: the row wasn't added.)
    // This is the authoritative moment: GC MUST fire and prune the orphan.
    await push([], ["ConfigSnapshot"]);

    // The payload is now pruned.
    expect(setup.state.kv.get(desiredKey)).toBeNull();
  });
});
