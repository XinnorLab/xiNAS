...
- **Roles are idempotent** — re-running `site.yml` over a healthy array converges (no reformat, no namespace rebuild). Destroying and rebuilding storage requires the explicit `xinas_storage_reset: true` plus an interactive `YES`; the legacy `xfs_force_mkfs` / `nvme_use_existing_namespaces` knobs no longer trigger wipes on their own. Reusing an existing array whose filesystem is foreign is a separate, narrower answer: the wizard shows what it found and asks before setting `xinas_fs_force_format`, which reformats the volume but keeps the arrays. See [docs/Installer/raid-spec.md](docs/Installer/raid-spec.md) §11
